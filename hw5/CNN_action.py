...
with rnn_action.as_default() as g:
    ...
    with tf.name_scope("attn_Score"):
        attn_score = ly.fully_connected(_feat , 256 , activation_fn=tf.nn.leaky_relu)
        attn_score = ly.fully_connected(attn_score , 1 , activation_fn=None)
        attn_score = tf.nn.softmax(attn_score , axis=1)
    
    with tf.name_scope("Extractor"):
        fc = ly.fully_connected(_feat , 1024 , activation_fn=tf.nn.leaky_relu)
        fc = ly.fully_connected(fc , 512 , activation_fn=tf.nn.leaky_relu)
    
    with tf.name_scope("weight_sum"):
        sum_feat = tf.reduce_sum(fc*attn_score , axis=1)
        
    with tf.name_scope("classifier"):
        
        fc_1 = ly.fully_connected(sum_feat , 256 , activation_fn=tf.nn.leaky_relu)
...
model_path = os.path.join(model_path , "resnet_avg_CNN.ckpt")
...
with open(os.path.join(result_path , "p1_valid.txt") , "w") as f:
    for tmp in record:
        f.write(str(tmp)+"\n")
